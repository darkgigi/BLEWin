...
class RTQRS(object):
    ...
    def __init__(self, sizeBuffer: int = 64, overlap:int=32, fs: float = 208, meanfilterorder: int = 3, envelopmentfilterorder: int = 2):
        """
        ...
        self.centroids = [0.0, 0.0]
        #self.countPlot = 0
        self.initCentroids = False
        self.countSamples = 0
        #self.qrsValues=0
        #self.nQRSValues = 0
        #self.kmean = KMeans(n_clusters=2)
        #self.kmean.fit([[0, 1],[0, 10]])
        self.centroids = [0, 100]
        self.a = 0
        self.b = 0
    ...
    def realTimeQRSDetection(self, buffer):
        ...
        locs = signal.find_peaks_cwt(qrs,np.arange(1,self.overlap))
        pks = qrs[locs]
        self.myBuffer = bfil
        ...
        if len(labels) > 0:
            ...
            while idx < len(pos):
                df = np.diff(qrs[pos[idx-1]:pos[idx]])
    ...
    def mean_filter(self, buffer):
        ...
        for element in range(msize,size+1):
            y[idx] = sum(buffer[0:idx+1])/element
            idx = idx + 1
        for element in range(size,lbuffer):
            y[idx] = sum(buffer[element-size:element])/size
            idx = idx + 1
        idx = len(y)
    ...
    def cond_min_eq(self, x): return x <= 0

    def cond_min(self, x): return x < 0

    def cond_may_eq(self, x): return x >= 0
